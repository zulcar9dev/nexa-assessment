...
echo "================================================"
echo "  BNI Kredit Konsumer - Setup Environment (Linux/macOS)"
echo "================================================"
echo ""
...
if [ ! -d "$DATA_DIR" ]; then
    ...
    # Konfigurasi port 5433 di postgresql.conf
...
if [ $? -eq 0 ]; then
    echo "[INFO] PostgreSQL sudah berjalan di port 5433."
else
    echo "[INFO] Memulai PostgreSQL di port 5433..."
    pg_ctl start -D "$DATA_DIR" -l "$TOOLS_DIR/pgsql/log.txt" -w -o "-p 5433"
    if [ $? -ne 0 ]; then
...
USER_EXISTS=$(psql -h localhost -p 5433 -U postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='bni_user'" 2>/dev/null)
if [ "$USER_EXISTS" = "1" ]; then
    echo "[INFO] User 'bni_user' sudah ada."
else
    echo "[INFO] Membuat user 'bni_user'..."
    psql -h localhost -p 5433 -U postgres -c "CREATE USER bni_user WITH PASSWORD 'bni_password' CREATEDB;"
fi

DB_EXISTS=$(psql -h localhost -p 5433 -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname='bni_kredit_konsumer'" 2>/dev/null)
if [ "$DB_EXISTS" = "1" ]; then
    echo "[INFO] Database 'bni_kredit_konsumer' sudah ada."
else
    echo "[INFO] Membuat database 'bni_kredit_konsumer'..."
    psql -h localhost -p 5433 -U postgres -c "CREATE DATABASE bni_kredit_konsumer OWNER bni_user;"
fi
...
echo "  1. Jalankan database: pg_ctl start -D $DATA_DIR -l $TOOLS_DIR/pgsql/log.txt -o \"-p 5433\""
