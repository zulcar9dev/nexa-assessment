#!/bin/bash
# =================================================
# Database Backup Script - Nexa Assessment
# =================================================
# 
# Usage:
#   Manual: ./scripts/backup-db.sh
#   Cron:   0 2 * * * /var/www/nexa-assessment/scripts/backup-db.sh >> /var/log/db-backup.log 2>&1
#
# Prerequisites:
...
BACKUP_DIR="${BACKUP_DIR:-$BASE_PATH/backups}"
...
PGPASSWORD="${DB_PASSWORD:-}" pg_dump \
    ...
    --no-owner \
    --no-acl \
    --verbose 2>/dev/null > "$BACKUP_DIR/$BACKUP_FILE" \
...
DELETED_COUNT=$(find "$BACKUP_DIR" -name "backup_*.sql" -mtime +"$KEEP_DAYS" -delete -print | wc -l)
