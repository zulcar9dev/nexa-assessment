...
import type { DSRResult } from "@/types/clients";
...
export function useCalculation() {
  ...
  const getPenghasilan = useCallback(
    (kategori: "type_a" | "type_b" | "type_c") => {
      if (kategori === "type_a") {
        ...
        if (formData.ung_kategori_pegawai === "non_dosen") {
          const remun30 = parseRupiah(formData.ung_remunerasi_30_bulanan || "0");
          const remun70 = parseRupiah(formData.ung_remunerasi_70_semesteran || "0");
        ...
        // PURNA
        const penghasilanMode = formData.type_b_penghasilan_mode || "minimum";
  ...
  const calculateAndUpdateDSR = useCallback(
    (kategori: "type_a" | "type_b" | "type_c") => {
      ...
      const limitPercentage = kategori === "type_c" ? 60 : 90;
      const limitMultiplier = kategori === "type_c" ? 0.6 : 0.9;
  ...
  const calculateAndUpdateDSRApi = useCallback(
    async (kategori: "type_a" | "type_b" | "type_c") => {
      // Get penghasilan
      const penghasilan = getPenghasilan(kategori);
