...
import { clientAktifSchema, getTabForField, formatValidationErrors } from "@/lib/validations";
...
export default function FormAktifPage() {
    // Store
    ...
    const dsrInputs = useFormStore(useShallow(state => ({
        ...
        additional_incomes: state.formData.additional_incomes, // [ADD] Penghasilan Tambahan
    ...
    useEffect(() => {
        calculateAndUpdateDSR("type_c");
    }, [
        dsrInputs.gaji_bulan_1_jumlah,
        dsrInputs.gaji_bulan_2_jumlah,
        dsrInputs.gaji_bulan_3_jumlah,
        dsrInputs.additional_incomes, // [ADD] Trigger recalculation on Tukin/Uang Makan changes
    ...
    const handleSave = async () => {
        ...
        const validation = clientAktifSchema.safeParse(currentFormData);
        ...
        const result = await submitForm("type_c", "SUBMITTED");
    ...
    const handleSaveDraft = async () => {
        ...
        const result = await submitForm("type_c", "DRAFT");
    ...
    const renderTabContent = () => {
        switch (currentTab) {
            ...
            case "tab-b":
                return <TabBPekerjaan kategori="type_c" />;
            case "tab-c":
                return <TabCPenghasilan kategori="type_c" />;
            case "tab-d":
                return <TabDSlik />;
            case "tab-e":
                return <TabEUsulan kategori="type_c" />;
            default:
                return <TabAIdentitas />;
        }
    };
