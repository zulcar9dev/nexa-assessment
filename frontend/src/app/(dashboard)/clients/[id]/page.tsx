...
const getKategori = (client: Client): string => {
    const kategori = String(client.kategori || "").toUpperCase();
...
export default function ClientDetailPage({
    ...
    const { id } = use(params);
    const { getDebitur, downloadDocx, isLoading, error } = useClient();
    const [client, setDebitur] = useState<Client | null>(null);
    ...
    const handleDownload = async () => {
        if (client) {
            await downloadDocx(id, client.applicantName);
        }
    };
        ...
        return (
            <div className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-bold text-primary-brand dark:text-[#a5b4fc]">
                        Detail Client
                    </h1>
                </div>
    ...
    const data = (client.dataLengkap || {}) as Record<string, any>;
    const kategori = getKategori(client);
    const segmentasi = String(client.segmentasi || data.segmentasi || "").toUpperCase();
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            {client.applicantName} - {kategoriLabels[kategori] || kategori}
                        </p>
                    </div>
                </div>
              ...
              rounded-lg font-medium transition-all duration-200"
                    ...
                    <Link
                        ...
                        className="flex items-center gap-2 px-4 py-2
              bg-primary-brand hover:bg-primary-brand-dark !text-white
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="card p-4 flex items-center gap-4">
                    <div className="w-12 h-12 bg-[#eef2ff] dark:bg-primary-brand/20 rounded-lg flex items-center justify-center">
                        <Calendar className="w-6 h-6 text-primary-brand dark:text-[#a5b4fc]" />
                    </div>
                    <div>
                ...
                <div className="card p-4 flex items-center gap-4">
                    <div className="w-12 h-12 bg-[#fff3e0] dark:bg-secondary-brand/20 rounded-lg flex items-center justify-center">
                    ...
                    <div className="p-4 space-y-3">
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium text-gray-800 dark:text-gray-200">
                                {data.nama_pemohon || client.applicantName || "-"}
                            </span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-gray-500 dark:text-gray-400">NIK</span>
                            <span className="font-mono text-gray-800 dark:text-gray-200">
                                {data.no_ktp_pemohon || client.idNumber || "-"}
                            </span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        {data.tempat_tinggal_berbeda === true || data.tempat_tinggal_berbeda === "true" ? (
                            <>
                                <div className="flex justify-between">
                                    <span className="text-gray-500 dark:text-gray-400">Alamat KTP</span>
                                    <span className="text-gray-800 dark:text-gray-200 text-right max-w-[200px]">
                                        {data.alamat_ktp || "-"}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-500 dark:text-gray-400">Alamat Tempat Tinggal</span>
                                    <span className="text-gray-800 dark:text-gray-200 text-right max-w-[200px]">
                            ...
                            <div className="flex justify-between">
                                <span className="text-gray-500 dark:text-gray-400">Alamat (KTP & Domisili)</span>
                                <span className="text-gray-800 dark:text-gray-200 text-right max-w-[200px]">
                                    {data.alamat_ktp || "-"}
                                </span>
                            </div>
                                    ...
                                    <>
                                        {data.estimasi_hak_pensiun && (
                                            <div className="flex justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
                                                ...
                                                <span className="font-bold text-primary-brand dark:text-[#a5b4fc]">
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-gray-700">
                                            ...
                                            <th className="text-right py-2 px-1 text-xs">Budget</th>
                                            <th className="text-right py-2 px-1 text-xs">Outstanding</th>
                                            <th className="text-right py-2 px-1 text-xs">Payment Schedule</th>
                                    ...
                                    <tbody>
                                        {data.slik_facilities.map((facility: { nama_bank?: string; plafon_maks?: string; outstanding?: string; angsuran?: string; kolektibilitas?: string; is_takeover?: boolean; is_topup_lunas?: boolean }, idx: number) => (
                                            <tr key={idx} className="border-b border-gray-100 dark:border-gray-800">
                                                ...
                                                <td className="py-2 px-1 text-right text-xs font-semibold text-primary-brand">
                ...
                <div className="card lg:col-span-2">
                    ...
                    <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div>
                            ...
                            <p className="text-lg font-bold text-gray-800 dark:text-gray-200 capitalize">
                                {String(client.jenisPengajuan || data.jenis_pengajuan || "-").replace(/_/g, " ")}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Budget Allocation</p>
                            <p className="text-xl font-bold text-primary-brand dark:text-[#a5b4fc]">
                                Rp {formatRupiah(data.usulan_plafon_kredit)}
                            </p>
                        </div>
                        <div>
                    ...
                    {data.tujuan_kredit && (
                        <div className="px-4 pb-4">
                            <p className="text-sm text-gray-500 dark:text-gray-400">Tujuan Assessment</p>
