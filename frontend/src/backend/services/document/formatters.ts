...
export function getCfmStatusPerkawinan(
  ...
  const noStr = noDokumen && noDokumen.trim() !== "" ? noDokumen.trim() : "xxx";
  const tglStr = tglDokumen && tglDokumen.trim() !== "" ? formatDateIndonesian(tglDokumen) : "xxx";
  const statusLower = status.toLowerCase().replace(/_/g, " ");

  switch (statusLower) {
    case "menikah":
      return `Pemohon berstatus Menikah Cfm. Kutipan Akta Nikah No. ${noStr} tanggal ${tglStr}.`;
    case "belum menikah":
    case "belum_menikah":
      return `Pemohon berstatus Belum Menikah Cfm. Surat Keterangan Belum Menikah No. ${noStr} tanggal ${tglStr}.`;
    case "cerai hidup":
    case "cerai_hidup":
      return `Pemohon berstatus Cerai Hidup Cfm. Akta Cerai No. ${noStr} tanggal ${tglStr}.`;
    case "cerai mati":
    case "cerai_mati":
      return `Pemohon berstatus Cerai Mati Cfm. Akta Kematian Pasangan No. ${noStr} tanggal ${tglStr}.`;
    default:
      return "";
  }
}
 ...
 */
export function getStatusKepegawaian(client: ClientData): string {
    const data = client.dataLengkap;
    const segmentasi = (client.segmentasi || data.segmentasi || "").toString().toLowerCase();
    const instansi = (data.instansi as string || "").trim();
    const instansiLower = instansi.toLowerCase();

    // Kategori: purna or prapurna
    const kategoriSlug = client.kategori.toLowerCase();
    ...
    // Fleksi Aktif Specific Defaults
    if (segmentasi === "pemerintahan") {
      return `Pemohon merupakan Pegawai Negeri Sipil (PNS) di ${instansi}.`;
    }

    if (segmentasi === "bumd_bumn" || segmentasi === "swasta") {
       return `Pemohon merupakan Pegawai di ${instansi}.`;
    }
 ...
 */
export function getMasaKerjaText(client: ClientData): string {
    const data = client.dataLengkap;
    const segmentasi = (client.segmentasi || data.segmentasi || "").toString().toLowerCase();
