...
export class FinancialContextBuilder {
  static build(
    ...
    const kategoriLower = String(client.kategori || "").toLowerCase();
    ...
    const activeIncomeComponents: Array<{ label: string; value: number }> = [];
    ...
    if (isAktif) {
      ...
      if (gajiValue > 0) {
        activeIncomeComponents.push({ label: "Gaji", value: gajiValue });
      }
      ...
      additionalIncomes.forEach((income) => {
        ...
        const val = calculateVarianceValue(add1, add2, add3);
        if (val > 0 && income.label) {
          activeIncomeComponents.push({ label: income.label, value: val });
          additionalValue += val;
        }
      });

      // Khusus PNS Non-Dosen UNG: Tambahkan Tunjangan/Remunerasi (30% bulanan + 70%/6 semesteran)
      if (data.ung_kategori_pegawai === "non_dosen") {
        const remun30 = parseInt(String(data.ung_remunerasi_30_bulanan || 0).replace(/[^0-9]/g, ""), 10) || 0;
        const remun70 = parseInt(String(data.ung_remunerasi_70_semesteran || 0).replace(/[^0-9]/g, ""), 10) || 0;
        const remunDiakui = remun30 + Math.round(remun70 / 6);
        if (remunDiakui > 0) {
          activeIncomeComponents.push({ label: "Tunjangan/Remunerasi", value: remunDiakui });
          additionalValue += remunDiakui;
        }
      }
      ...
      // Also parse active salary for Prapurna to support dynamic investigasi text
      const prapurnaGaji1 = parseInt(String(data.gaji_bulan_1_jumlah || 0).replace(/[^0-9]/g, ""), 10) || 0;
      const prapurnaGaji2 = parseInt(String(data.gaji_bulan_2_jumlah || 0).replace(/[^0-9]/g, ""), 10) || 0;
      const prapurnaGaji3 = parseInt(String(data.gaji_bulan_3_jumlah || 0).replace(/[^0-9]/g, ""), 10) || 0;
      const prapurnaGajiVal = calculateVarianceValue(prapurnaGaji1, prapurnaGaji2, prapurnaGaji3);
      if (prapurnaGajiVal > 0) {
        activeIncomeComponents.push({ label: "Gaji", value: prapurnaGajiVal });
      }
    }
    ...
    return {
      ...
      // UNG PNS Non-Dosen Specific
      ung_kategori_pegawai: data.ung_kategori_pegawai || "",
      ung_remun_30: formatRupiah(parseInt(String(data.ung_remunerasi_30_bulanan || 0).replace(/[^0-9]/g, ""), 10) || 0),
      ung_remun_70: formatRupiah(parseInt(String(data.ung_remunerasi_70_semesteran || 0).replace(/[^0-9]/g, ""), 10) || 0),
      ung_remun_diakui_bulanan: formatRupiah((parseInt(String(data.ung_remunerasi_30_bulanan || 0).replace(/[^0-9]/g, ""), 10) || 0) + Math.round((parseInt(String(data.ung_remunerasi_70_semesteran || 0).replace(/[^0-9]/g, ""), 10) || 0) / 6)),
      is_ung_non_dosen: data.ung_kategori_pegawai === "non_dosen",
      ...
      // Dynamic income components and flags
      active_income_components: activeIncomeComponents,
      has_gaji: gajiValue > 0,
      has_tunjangan: additionalValue > 0,
