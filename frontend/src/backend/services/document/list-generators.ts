...
export class ListGenerators {
  ...
  static generateKerabatPrapurnaList(context: Record<string, unknown>): Record<string, string>[] {
    const list: string[] = [];
    const statusKepegawaian = String(context.status_kepegawaian || "Calon Pensiunan PNS"); 
    const instansi = String(context.instansi || "-");
    const statusRumah = context.status_rumah || "-";

    list.push(formatStatusKepegawaian(statusKepegawaian, instansi));
    list.push(`Menurut Ybs. rumah yang di tempati Pemohon saat ini adalah Rumah ${statusRumah}.`);
    list.push("Ybs. menyampaikan bahwa Pemohon memiliki kemampuan untuk menyetor angsuran atas kredit yang dimohon, dan Ybs bersedia untuk mengingatkan Pemohon untuk kewajiban angsuran perbulan"); 
