...
export class ListGenerators {
  ...
  static generateAktifInvestigasiList(
    ...
    // 1. Alamat KTP
    list.push(`Alamat Pemohon sesuai KTP di ${alamatKtp}`); // Removed trailing dot to match most samples (or keep it if standard requires)
    ...
    // P3K Logic Detection
    const isP3K = /p3k|pppk/i.test(manualStatus);
    const skLabel = isP3K
      ? "Surat Keputusan (SK) Pengangkatan PPPK"
      : "Surat Keputusan (SK) Pengangkatan Pegawai";
    const skLabelSwasta = isP3K ? "SK Pengangkatan PPPK" : "SK Pengangkatan";
    const skLabelPemerintahan = isP3K ? "SK Pengangkatan PPPK" : "SK";
    ...
    const segmentasi =
      config?.segmentasi ||
      String(context.segmentasi || "SWASTA").toUpperCase();
    ...
    if (isBumn) {
      ...
      const tglSk = context.tgl_sk_cpns || "-";
      list.push(
        `Pemohon adalah ${manualStatus} pada ${instansi} Cfm. ${skLabelSwasta} No ${noSk} tanggal ${tglSk}.`,
      );
    }
    ...
    if (useBup) {
      if (config?.infoPensiun) {
        ...
        const tglPensiun = context.tgl_pensiun || "-";
        // UNG Format: Pemohon akan memasuki Batas Usia Pensiun per Tanggal ...
        list.push(
          `Pemohon akan memasuki Batas Usia Pensiun per Tanggal ${tglPensiun} Cfm. Estimasi Hak Tabungan Hari Tua dan Pensiun Pokok.`,
        );
      }
    ...
    list.push(
      `Maksud mengajukan Fasilitas Kredit Fleksi ${jenis} sebesar Rp. ${plafon} Jangka Waktu ${tenor} Bulan.`,
    );

    // 14. Tujuan
    const tujuan = context.tujuan_kredit || "-";
    list.push(`Tujuan penggunaan Kredit untuk ${tujuan}`);
  ...
  static generateRpcPurnaList(
    ...
    // Header
    list.push(
      `Penghasilan Pemohon Cfm. Rekening Koran Gaji di ${context.payroll_bank} nomor ${context.payroll_no_rek} atas nama ${context.nama_pemohon} dengan data sebagai berikut :`,
    );
    ...
    const gajiLabel =
      penghasilanMode === "langsung"
        ? "Gaji Pensiun Yang Di Akui"
        : `Gaji Pensiun ${context.purna_bulan_penghasilan_nama} ${modeKeterangan}`;

    list.push(`${gajiLabel}\t\t:  Rp. ${context.purna_bulan_penghasilan},-.`);
    list.push(`Penghasilan Pemohon\t\t\t:  Rp. ${context.rpc_penghasilan},-.`);
    list.push(`DSC 90 %\t\t\t\t:  Rp. ${context.rpc_dsc_90},-.`);
    ...
    // SLIK
    list.push(
      `Cfm. Info SLIK Ideb posisi terakhir Tanggal ${context.tgl_slik} Pemohon memiliki Fasilitas Kredit sebagai berikut :`,
    );

    // Fasilitas Kredit Loop
    if (slikFacilities && slikFacilities.length > 0) {
      slikFacilities.forEach((f: Record<string, unknown>) => {
        list.push(
          `-Fasilitas Kredit ${f.jenis_kredit} di ${f.nama_bank} maks Rp. ${f.plafon_maks} outs Rp. ${f.outstanding} angsuran Rp. ${f.angsuran} Coll ${f.kolektibilitas}. ${f.alasan}`,
        );
      });
    } else {
      list.push(`-Nihil - Tidak ada fasilitas kredit`);
    }
    ...
    list.push(
      `Maksimal Angsuran Kredit yang dapat diberikan sampai dengan : \tRp. ${context.rpc_maksimal_angsuran},-`,
    );
    list.push(
      `Angsuran Kredit yang dapat diusulkan       \t\t\t: \tRp. ${context.rpc_angsuran_diusulkan},-`,
    );
    list.push(
      `Total Angsuran Kredit Eksisting & Angsuran yang diusul   \t: \tRp. ${context.rpc_total_angsuran_baru},-`,
    );
    list.push(``);
  ...
  static generateRpcPrapurnaList(
    ...
    // Penghasilan
    list.push(
      `Estimasi Gaji (BUP) \t: \tRp. ${context.estimasi_hak_pensiun},-`,
    );
    list.push(`Penghasilan Pemohon \t: \tRp. ${context.rpc_penghasilan},-`);
    list.push(`DSC 90 %    \t\t: \tRp. ${context.rpc_dsc_90},-`);
  ...
  static generateRpcAktifList(
    ...
    list.push(
      `Penghasilan Pemohon Cfm. Slip Gaji dan Rekening Koran Gaji Pemohon di BNI Nomor Rekening ${context.aktif_nomor_rekening_gaji} atas nama ${context.nama_pemohon} dengan data sebagai berikut :`,
    );
    list.push(``);
    ...
    */
    ...
    if (gaji1 > 0 || gaji2 > 0 || gaji3 > 0) {
      ...
      list.push(
        `Gaji Bulan ${gajiBulanNama} (${gajiMethodText})\t\t: Rp. ${gajiVarianceStr},-`,
      );
    }
    ...
    // === TOTAL & DSC ===
    list.push(
      `Penghasilan Calon Debitur\t\t: Rp. ${context.aktif_penghasilan_calon_debitur},-`,
    );
    list.push(
    ...
    if (context.aktif_dsc_percent === "90%") {
      list.push(``);
      list.push(
        `Relaksasi CP - DSR 90% : Hanya Memperhitungkan Kewajiban yang berjalan di Bank BNI dengan tetap Mempertimbangkan Kualitas SLIK Pemohon secara Keseluruhan baik di Bank BNI maupun Bank lain. Cfm Lampiran Memo Divisi CRP No. CRP/4.1/0231 tanggal 13 Januari 2026 Perihal Juklak Program Relaksasi BNI Fleksi Corporate Client.`,
      );
    }
    ...
    // Kalkulasi
    list.push(
      `Total Angsuran Calon Debitur\t\t\t: Rp. ${context.aktif_total_angsuran_calon_debitur},-`,
    );
    list.push(
      `Maksimal Angsuran Kredit yang dapat diberikan s.d\t: Rp. ${context.aktif_maksimal_angsuran},-`,
    );
    list.push(
      `Angsuran Kredit yang dapat diusulkan\t\t\t: Rp. ${context.aktif_angsuran_diusulkan},-`,
    );
    list.push(
      `Total Angsuran Kredit Eksisting & Angsuran yg diusul\t: Rp. ${context.aktif_total_angsuran_all},-`,
    );
    list.push(``);
  ...
  static generateVerifikasiPenghasilanAktifList(
    ...
    list.push(
      `Penghasilan Pemohon Cfm. Rekening Koran Gaji ${context.payroll_bank} nomor ${context.payroll_no_rek} atas nama ${context.nama_pemohon} dengan data sebagai berikut :`,
    );
    ...
    // We check these first or after? Plan said dynamic first, but let's do legacy first to keep structure if used.
    ...
    if (Array.isArray(additionalIncomes)) {
      for (const income of additionalIncomes) {
        // Safe access and formatting
        // We assume logic: if it has label, we show it.
        if (income.label) {
          // Check if we need to format values.
          // The context values might come raw from UI state if not pre-processed.
          // But usually template context builder handles basic mapping.
          // For complex arrays, we might need to handle it here or in template-context builder.
          // Let's assume we handle it here to be safe.
          ...
          const b1 = formatVal(income.bulan_1);
          const b2 = formatVal(income.bulan_2);
          const b3 = formatVal(income.bulan_3);

          list.push(
            `-${income.label} ${income.bulan_1_nama || "Bulan 1"}\t:\tRp. ${b1},-`,
          );
          list.push(
            `-${income.label} ${income.bulan_2_nama || "Bulan 2"}\t:\tRp. ${b2},-`,
          );
          list.push(
            `-${income.label} ${income.bulan_3_nama || "Bulan 3"}\t:\tRp. ${b3},-`,
          );
        }
      }
    }
